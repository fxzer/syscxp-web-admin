<template>
  <div v-loading.fullscreen.lock="wrapperLoading" element-loading-text="拼命加载中"
        ...
        <el-table-column type="expand" width="40">
          <template v-if="row" slot-scope="{ row }">
            <div style="padding: 5px 20px;">
              <div>
                <el-button type="primary" style="margin-bottom: 5px;" size="small" @click="openAddOrEditItem({})">新增优势</el-button>
              </div>
    ...
    <AddEditAdv :visible.sync="addEditVisible"  :currentRow="currentRow" @done="addEditDone"/>
    <AddEditAdvItem :visible.sync="addEditItemVisible" :currentRow="currentItem" @done="addEditItemDone" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async queryList() {
      ...
      const result = await queryLand({
        conditions:[{name:'type',op:'=',value:'advantages'}]
      })
  ...
  components: {
    AddEditAdv: () => import('./components/AddEditAdv.vue'),
    AddEditAdvItem: () => import('./components/AddEditAdvItem.vue'),
  },
  computed: {
    ...mapState([
      "cellLayout",
